"""
quantiza_nao_uniforme.py — Script CLI para Quantização Não-Uniforme com K-Means.

Uso:
    python scripts/quantiza_nao_uniforme.py <caminho_da_imagem> <bits>

Exemplo:
    python scripts/quantiza_nao_uniforme.py imagem.png 4

Técnica:
    Quantização Não-Uniforme via K-Means — encontra os 2^bits centróides ótimos
    baseados na distribuição real dos pixels da imagem.
"""
...
def main(image_path: str, bits: int) -> None:
    """
    Executa a quantização K-Means em uma imagem e salva o resultado.
    ...
    n_tons = 2 ** bits
    print(f"\nAplicando {technique_label(QuantizationTechnique.KMEANS)} ({bits} bits / {n_tons} tons)...")
    print("  (K-Means pode demorar alguns segundos para imagens grandes)")
...
def _parse_args() -> tuple[str, int]:
    ...
    if len(sys.argv) != 3:
        print("Uso: python scripts/quantiza_nao_uniforme.py <caminho_da_imagem> <bits (1-8)>")
        sys.exit(1)
